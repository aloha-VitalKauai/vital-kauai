...
const PHQ9_QUESTIONS = [
  ...
  'Moving or speaking so slowly that other people could have noticed \u2014 or the opposite, being so fidgety or restless that you have moved around more than usual',
...
interface Props {
  ...
  onUpdateResponse: (field: string, value: number) => void;
  readOnly: boolean;
}

export function Phq9Step({ responses, onUpdateResponse, readOnly }: Props) {
  const scores = PHQ9_QUESTIONS.map((_, i) => responses[`phq9_q${i + 1}`] ?? null);
  const answered = scores.filter(s => s !== null).length;
  const total = scores.reduce<number>((sum, s) => sum + (s ?? 0), 0);
  const allDone = answered === 9;

  function handleSelect(questionIdx: number, score: number) {
    const field = `phq9_q${questionIdx + 1}`;
    onUpdateResponse(field, score);

    const next = [...scores];
    next[questionIdx] = score;
    if (next.every(s => s !== null)) {
      const sum = next.reduce<number>((a, b) => a + (b ?? 0), 0);
      onUpdateResponse('phq9_total', sum);
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', marginTop: '2rem', paddingTop: '1.5rem', borderTop: `1px solid ${T.borderLight}` }}>
        <span style={{ fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: T.creamMuted }}>PHQ-9 score</span>
        <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.4rem', color: T.creamDim }}>
          {allDone ? total : answered > 0 ? `${total}+` : '\u2014'}
        </span>
        <span style={{ fontSize: '0.75rem', color: T.creamMuted }}>/ 27</span>
      </div>
    </div>
  );
}
