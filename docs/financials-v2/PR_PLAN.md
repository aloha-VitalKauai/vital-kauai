...
123. **Ordinary exception creation succeeds** as `service_role` using only the granted columns; the row lands `open` with every resolution, quarantine and release column `NULL`.
124. **Supplying any protected lifecycle column at `INSERT` fails as `service_role`** — each of `resolution_status` (non-`open`), `resolved_at`, `resolved_by`, `resolution_note`, `quarantined_at`, `quarantine_reason`, `released_at`, `released_by` and `release_note` is rejected, tested individually. The `BEFORE INSERT` trigger rejects them **even when the grant is widened**, proving the trigger and not the grant is load-bearing.
...
133. **Agreement creation requires its initial lifecycle event** — inserting an agreement with no lifecycle event fails **at commit**; inserting both in one transaction succeeds in either order; `finance.create_agreement()` succeeds unchanged.
