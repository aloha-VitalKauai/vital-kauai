...
- [ ] **Guarded transitions are guarded at `INSERT` too** — a row cannot be created already in a state that a function or `UPDATE` restriction is meant to control, and the guard holds for `service_role`.
