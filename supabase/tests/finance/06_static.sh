...
chk "req 2: the catalog fingerprint of the finance schema matches expected_objects.txt exactly [ST-001]" \
...
chk "the rebaseline script is token-gated and the gate never invokes it [ST-002]" \
  "grep -q 'I-REVIEWED-THE-DIFF' supabase/tests/rebaseline_census.sh && ! grep -q 'rebaseline_census' supabase/tests/harness_gate.sh"
chk "the gate hashes the baseline before and after every run (see harness_gate.sh) [ST-003]" \
  "grep -q 'BASELINE_HASH_BEFORE' supabase/tests/harness_gate.sh"
chk "req 2: the census is substantive, not an empty or truncated file [ST-004]" \
...
chk "req 1: all 8 migrations exist and the finance schema they build is present in this database [ST-005]" \
...
chk "req 3: run_all.sh executes 'dropdb --if-exists' and ON_ERROR_STOP=1 as real tokens (bash-aware lex, comments can never satisfy) [ST-006]" \
...
chk "req 69: every aggregate view depends on v_agreement_balances in pg_depend [ST-007]" \
...
chk "req 69: no aggregate view re-implements the balance formula independently [ST-008]" \
...
chk "req 70 / D-074: exactly two lifecycle derivations exist (1 view + 1 routine) [ST-009]" \
...
chk "req 70: the consumer projection v_agreement_lifecycle exists as a view [ST-010]" \
  "[ \$(q \"select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace
      where n.nspname='finance' and c.relname='v_agreement_lifecycle' and c.relkind='v'\") -eq 1 ]"
chk "req 70: the only enforcement derivation is tg_lifecycle_transition [ST-011]" \
...
chk "req 70: v_agreement_lifecycle carries security_invoker in its catalog reloptions [ST-012]" \
...
chk "finding 11: migration 0001 asserts the PostgreSQL major version (comments stripped) [ST-013]" \
  "\$STRIP supabase/migrations/20260730000001_finance_harden_is_founder.sql | grep -q 'server_version_num'"
chk "blocker 1: migration 0001 executes is_founder() after hardening it (comments stripped) [ST-014]" \
  "\$STRIP supabase/migrations/20260730000001_finance_harden_is_founder.sql | grep -q 'public.is_founder()'"
chk "blocker 1 (catalog): is_founder carries the pinned search_path the migration set [ST-015]" \
...
chk "blocker 2: rollback resets the is_founder search_path (comments stripped) [ST-016]" \
...
chk "no finance object has a foreign key into a legacy financial table [ST-017]" \
...
chk "no migration writes a legacy financial table (stripper rc checked; scan covers the full canonical list) [ST-018]" \
  "legacy_write_scan"
chk "no finance routine body references a legacy financial table (catalog) [ST-019]" \
  "[ \$(q \"select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
...
chk "anon holds no privilege anywhere in the finance schema [ST-020]" \
