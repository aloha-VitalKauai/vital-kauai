...
select throws_ok($$ select finance.create_agreement('aaaaaaaa-0000-0000-0000-00000000000a',null,'membership','  ') $$,
  null, null, 'test 71: blank reason raises');
select throws_ok($$ select finance.create_agreement('aaaaaaaa-0000-0000-0000-00000000000a','cccccccc-0000-0000-0000-00000000000c','journey_contribution','dup') $$,
  null, null, 'test 17: duplicate (member,journey,purpose) raises');
...
select throws_ok($$
  ...
  end $b$; $$, null, null, 'test 133: agreement with no initial event fails the deferred completeness check');

-- test 133b: child-first is rejected (FK is non-deferrable)
select throws_ok($$ insert into finance.agreement_lifecycle_events
    (agreement_id,from_status,to_status,reason,actor_id)
    values ('dddddddd-0000-0000-0000-00000000000d',null,'draft','x','11111111-1111-1111-1111-111111111111') $$,
  null, null, 'test 133b: child-first insert is rejected');
...
select throws_ok($$ insert into finance.agreement_lifecycle_events (agreement_id,from_status,to_status,reason,actor_id)
  select id,'active','draft','back','11111111-1111-1111-1111-111111111111' from ag $$,
  null,null,'test 72: active->draft rejected');
select throws_ok($$ insert into finance.agreement_lifecycle_events (agreement_id,from_status,to_status,reason,actor_id)
  select id,'draft','active','stale','11111111-1111-1111-1111-111111111111' from ag $$,
  null,null,'test 72: stale from_status rejected');
select lives_ok($$ insert into finance.agreement_lifecycle_events (agreement_id,from_status,to_status,reason,actor_id)
...
select throws_ok($$ insert into finance.agreement_lifecycle_events (agreement_id,from_status,to_status,reason,actor_id)
  select id,'canceled','active','undo','11111111-1111-1111-1111-111111111111' from ag $$,
  null,null,'test 72: canceled is terminal');
...
select throws_ok($$ update finance.agreement_amounts set amount_cents=1 $$, null,null,'test 6: amounts cannot be UPDATEd');
select throws_ok($$ delete from finance.agreement_amounts $$, null,null,'test 6: amounts cannot be DELETEd');
select throws_ok($$ update finance.agreement_lifecycle_events set reason='x' $$, null,null,'test 7: lifecycle events cannot be UPDATEd');
select throws_ok($$ delete from finance.agreement_lifecycle_events $$, null,null,'test 7: lifecycle events cannot be DELETEd');

-- test 15: future-dated amendment rejected
select throws_ok($$ insert into finance.agreement_amounts (agreement_id,amount_cents,effective_at,reason,actor_id)
  select id, 1, now()+interval '1 day','future','11111111-1111-1111-1111-111111111111' from ag $$,
  null,null,'test 15: future-dated amendment rejected');
...
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_payment_intent_id,occurred_at,livemode)
  select id,'stripe_payment',5000,'stripe',null,now(),true from ag $$,
  null,null,'test 28 (L1): stripe_payment without payment-intent id rejected');
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,external_method,occurred_at,livemode,reason,recorded_by)
  select id,'external_payment',5000,'external',null,now(),true,'r','11111111-1111-1111-1111-111111111111' from ag $$,
  null,null,'test 29 (L2): external_payment without method rejected');
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,external_method,occurred_at,livemode)
  select id,'external_payment',5000,'external','cash',now(),true from ag $$,
  null,null,'test 29 (L12): external_payment with no attribution rejected');
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_payment_intent_id,external_method,occurred_at,livemode)
  select id,'stripe_payment',5000,'stripe','pi_x','cash',now(),true from ag $$,
  null,null,'test 32c (L13): stripe entry carrying external_method rejected');
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,external_method,provider_payment_intent_id,occurred_at,livemode,reason,recorded_by)
  select id,'external_payment',5000,'external','cash','pi_x',now(),true,'r','11111111-1111-1111-1111-111111111111' from ag $$,
  null,null,'test 32d (L13): external entry carrying a payment-intent id rejected');
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,external_method,occurred_at,livemode,reason,recorded_by,recorded_by_system)
  select id,'external_payment',5000,'external','cash',now(),true,'r','11111111-1111-1111-1111-111111111111','legacy_import' from ag $$,
  null,null,'test 32 (L12): both human and system attribution rejected');
...
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_object_id,provider_payment_intent_id,occurred_at,livemode)
  select id,'stripe_payment',1,'stripe','ch_1','pi_2',now(),true from ag $$,
  null,null,'test 36 (L8): duplicate provider object rejected');
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_object_id,provider_payment_intent_id,occurred_at,livemode)
  select id,'stripe_payment',1,'stripe','ch_2','pi_1',now(),true from ag $$,
  null,null,'test 37 (L8b): duplicate payment intent rejected');

-- refunds
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_object_id,parent_entry_id,occurred_at,livemode)
  select a.id,'refund',-200000,'stripe','re_big',p.id,now(),true from ag a, pay p $$,
  null,null,'test 41 (L7): refund exceeding settled amount rejected');
...
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,parent_entry_id,occurred_at,livemode,reason,recorded_by)
  select a.id,'reversal',-100000,'stripe',p.id,now(),true,'undo','11111111-1111-1111-1111-111111111111' from ag a, pay p $$,
  null,null,'test 45 (L6): reversal rejected while parent has an unreversed child');
...
select throws_ok($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,parent_entry_id,occurred_at,livemode,reason,recorded_by)
  select a.id,'reversal',-100000,'stripe',p.id,now(),true,'again','11111111-1111-1111-1111-111111111111' from ag a, pay p $$,
  null,null,'test 47: an entry cannot be reversed twice');
...
-- ============ run state machine ============
select throws_ok($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,status,window_exhausted,finished_at)
  values (true,'v1',now()-interval '1 day',now(),true,'completed',false,now()) $$,
  null,null,'test 84: completed with window_exhausted=false rejected');
select throws_ok($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,status,window_exhausted,finished_at)
  values (true,'v1',now()-interval '1 day',now(),true,'partial',true,now()) $$,
  null,null,'test 84: partial with window_exhausted=true rejected');
select throws_ok($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,status,window_exhausted)
  values (true,'v1',now()-interval '1 day',now(),true,'running',true) $$,
  null,null,'test 84: running with window_exhausted=true rejected');
...
select throws_ok($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,status,window_exhausted,finished_at)
  values (true,'v1',now()-interval '1 day',now(),true,'running',false,now()) $$,
  null,null,'test 83: running carrying finished_at rejected');

-- test 107: implementation_version required
select throws_ok($$ insert into finance.reconciliation_runs (livemode,window_start,window_end,dry_run)
  values (true,now()-interval '1 day',now(),true) $$,
  null,null,'test 107: implementation_version is required');

-- test 129: approval fields cannot be inserted
select throws_ok($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,approved_by,approved_at,approval_note)
  values (true,'v1',now()-interval '1 day',now(),true,'11111111-1111-1111-1111-111111111111',now(),'sneak') $$,
  null,null,'test 129: a run cannot be created already approved');
...
select throws_ok($$ select finance.approve_dry_run((select id from dry),'ok') $$,
  null,null,'test 104: approval without a completed report raises');
...
select throws_ok($$ select finance.approve_dry_run((select id from dry),'again') $$,
  null,null,'test 103: a second approval raises');
select throws_ok($$ select finance.approve_dry_run((select id from dry),'  ') $$,
  null,null,'test 114: blank approval note raises');

-- test 105/115: approved evidence is frozen
select throws_ok($$ update finance.reconciliation_runs set window_end=now()+interval '1 day' where id=(select id from dry) $$,
  null,null,'test 105: window_end frozen after approval');
select throws_ok($$ update finance.reconciliation_runs set report_version='r2' where id=(select id from dry) $$,
  null,null,'test 105: report frozen after approval');
select throws_ok($$ update finance.reconciliation_runs set implementation_version='v2' where id=(select id from dry) $$,
  null,null,'test 105: implementation_version frozen after approval');
...
select throws_ok($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id,resolution_status,resolved_at,resolved_by,resolution_note)
  values ('amount_mismatch',true,'ch_10','resolved',now(),'11111111-1111-1111-1111-111111111111','pre') $$,
  null,null,'test 124: cannot create an exception already resolved');
select throws_ok($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id,quarantined_at,quarantine_reason)
  values ('amount_mismatch',true,'ch_11',now(),'x') $$,
  null,null,'test 124: cannot create an exception already quarantined');

-- test 102: dedup_key is generated and cannot be supplied
select throws_ok($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id,dedup_key)
  values ('amount_mismatch',true,'ch_12','ATTACK') $$,
  null,null,'test 102: dedup_key cannot be supplied by the writer');
...
select throws_ok($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id)
  values ('amount_mismatch',true,'ch_9') $$,
  null,null,'test 79: a second open exception with the same identity is rejected');

-- test 99: provider_object_processing_failed shape
select throws_ok($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id)
  values ('provider_object_processing_failed',true,'ch_20') $$,
  null,null,'test 99: processing-failure without detail rejected');
...
select throws_ok($$ select finance.resolve_exception((select id from exc),'open','x') $$,
  null,null,'test 119: resolving to open raises');
select throws_ok($$ select finance.resolve_exception((select id from exc),'resolved','  ') $$,
  null,null,'test 119: blank resolution note raises');
...
select throws_ok($$ select finance.resolve_exception((select id from exc),'dismissed','again') $$,
  null,null,'test 119: repeat resolution raises');
