...
select denied($$ select finance.create_agreement('aaaaaaaa-0000-0000-0000-00000000000a',null,'membership','  ') $$, 'P0001', 'create_agreement: a non-blank reason is required', 'test 71: blank reason raises');
select denied($$ select finance.create_agreement('aaaaaaaa-0000-0000-0000-00000000000a','cccccccc-0000-0000-0000-00000000000c','journey_contribution','dup') $$, '23505', 'agreements_member_journey_purpose_key', 'test 17: duplicate (member,journey,purpose) raises');
...
select denied($$
  ...
  end $b$; $$, 'P0001', 'agreement b3f43a33-f781-4866-b7ca-8b13a1491a73 must have exactly one', 'test 133: agreement with no initial event fails the deferred completeness check');
...
select denied($$ insert into finance.agreement_lifecycle_events
    ...
    values ('dddddddd-0000-0000-0000-00000000000d',null,'draft','x','11111111-1111-1111-1111-111111111111') $$, 'P0001', 'agreement dddddddd-0000-0000-0000-00000000000d does not exist', 'test 133b: child-first insert is rejected');
...
select denied($$ insert into finance.agreement_lifecycle_events (agreement_id,from_status,to_status,reason,actor_id)
  select id,'active','draft','back','11111111-1111-1111-1111-111111111111' from ag $$, 'P0001', 'illegal lifecycle transition active -> draft', 'test 72: active->draft rejected');
select denied($$ insert into finance.agreement_lifecycle_events (agreement_id,from_status,to_status,reason,actor_id)
  select id,'draft','active','stale','11111111-1111-1111-1111-111111111111' from ag $$, 'P0001', 'stale transition: from_status draft but current status is active', 'test 72: stale from_status rejected');
select lives_ok($$ insert into finance.agreement_lifecycle_events (agreement_id,from_status,to_status,reason,actor_id)
...
select denied($$ insert into finance.agreement_lifecycle_events (agreement_id,from_status,to_status,reason,actor_id)
  select id,'canceled','active','undo','11111111-1111-1111-1111-111111111111' from ag $$, 'P0001', 'illegal lifecycle transition canceled -> active', 'test 72: canceled is terminal');
...
select denied($$ update finance.agreement_amounts set amount_cents=1 $$, 'P0001', 'UPDATE on agreement_amounts is forbidden: agreement_amounts is an', 'test 6: amounts cannot be UPDATEd');
select denied($$ delete from finance.agreement_amounts $$, 'P0001', 'DELETE on agreement_amounts is forbidden: agreement_amounts is an', 'test 6: amounts cannot be DELETEd');
select denied($$ update finance.agreement_lifecycle_events set reason='x' $$, 'P0001', 'UPDATE on agreement_lifecycle_events is forbidden:', 'test 7: lifecycle events cannot be UPDATEd');
select denied($$ delete from finance.agreement_lifecycle_events $$, 'P0001', 'DELETE on agreement_lifecycle_events is forbidden:', 'test 7: lifecycle events cannot be DELETEd');
...
select denied($$ insert into finance.agreement_amounts (agreement_id,amount_cents,effective_at,reason,actor_id)
  select id, 1, now()+interval '1 day','future','11111111-1111-1111-1111-111111111111' from ag $$, 'P0001', 'future-dated amendment rejected: effective_at 2026-08-02', 'test 15: future-dated amendment rejected');
...
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_payment_intent_id,occurred_at,livemode)
  select id,'stripe_payment',5000,'stripe',null,now(),true from ag $$, '23514', 'ledger_entries', 'test 28 (L1): stripe_payment without payment-intent id rejected');
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,external_method,occurred_at,livemode,reason,recorded_by)
  select id,'external_payment',5000,'external',null,now(),true,'r','11111111-1111-1111-1111-111111111111' from ag $$, '23514', 'ledger_entries', 'test 29 (L2): external_payment without method rejected');
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,external_method,occurred_at,livemode)
  select id,'external_payment',5000,'external','cash',now(),true from ag $$, '23514', 'ledger_entries', 'test 29 (L12): external_payment with no attribution rejected');
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_payment_intent_id,external_method,occurred_at,livemode)
  select id,'stripe_payment',5000,'stripe','pi_x','cash',now(),true from ag $$, '23514', 'ledger_entries', 'test 32c (L13): stripe entry carrying external_method rejected');
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,external_method,provider_payment_intent_id,occurred_at,livemode,reason,recorded_by)
  select id,'external_payment',5000,'external','cash','pi_x',now(),true,'r','11111111-1111-1111-1111-111111111111' from ag $$, '23514', 'ledger_entries', 'test 32d (L13): external entry carrying a payment-intent id rejected');
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,external_method,occurred_at,livemode,reason,recorded_by,recorded_by_system)
  select id,'external_payment',5000,'external','cash',now(),true,'r','11111111-1111-1111-1111-111111111111','legacy_import' from ag $$, '23514', 'ledger_entries', 'test 32 (L12): both human and system attribution rejected');
...
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_object_id,provider_payment_intent_id,occurred_at,livemode)
  select id,'stripe_payment',1,'stripe','ch_1','pi_2',now(),true from ag $$, '23505', 'ledger_entries_provider_object_uq', 'test 36 (L8): duplicate provider object rejected');
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_object_id,provider_payment_intent_id,occurred_at,livemode)
  select id,'stripe_payment',1,'stripe','ch_2','pi_1',now(),true from ag $$, '23505', 'ledger_entries_payment_intent_uq', 'test 37 (L8b): duplicate payment intent rejected');
...
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,provider_object_id,parent_entry_id,occurred_at,livemode)
  select a.id,'refund',-200000,'stripe','re_big',p.id,now(),true from ag a, pay p $$, 'P0001', 'L7: cumulative refunds 200000 exceed settled amount 100000 on entry', 'test 41 (L7): refund exceeding settled amount rejected');
...
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,parent_entry_id,occurred_at,livemode,reason,recorded_by)
  select a.id,'reversal',-100000,'stripe',p.id,now(),true,'undo','11111111-1111-1111-1111-111111111111' from ag a, pay p $$, 'P0001', 'L6: parent 2635a3e4-3553-4238-accf-d06c6d5d24f2 has 1 unreversed', 'test 45 (L6): reversal rejected while parent has an unreversed child');
...
select denied($$ insert into finance.ledger_entries (agreement_id,entry_type,amount_cents,source,parent_entry_id,occurred_at,livemode,reason,recorded_by)
  select a.id,'reversal',-100000,'stripe',p.id,now(),true,'again','11111111-1111-1111-1111-111111111111' from ag a, pay p $$, 'P0001', 'L6: parent 2635a3e4-3553-4238-accf-d06c6d5d24f2 has 1 unreversed', 'test 47: an entry cannot be reversed twice');
...
select denied($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,status,window_exhausted,finished_at)
  values (true,'v1',now()-interval '1 day',now(),true,'completed',false,now()) $$, '23514', 'reconciliation_runs', 'test 84: completed with window_exhausted=false rejected');
select denied($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,status,window_exhausted,finished_at)
  values (true,'v1',now()-interval '1 day',now(),true,'partial',true,now()) $$, '23514', 'reconciliation_runs', 'test 84: partial with window_exhausted=true rejected');
select denied($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,status,window_exhausted)
  values (true,'v1',now()-interval '1 day',now(),true,'running',true) $$, '23514', 'reconciliation_runs', 'test 84: running with window_exhausted=true rejected');
...
select denied($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,status,window_exhausted,finished_at)
  values (true,'v1',now()-interval '1 day',now(),true,'running',false,now()) $$, '23514', 'reconciliation_runs', 'test 83: running carrying finished_at rejected');
...
select denied($$ insert into finance.reconciliation_runs (livemode,window_start,window_end,dry_run)
  values (true,now()-interval '1 day',now(),true) $$, '23502', 'implementation_version', 'test 107: implementation_version is required');
...
select denied($$ insert into finance.reconciliation_runs (livemode,implementation_version,window_start,window_end,dry_run,approved_by,approved_at,approval_note)
  values (true,'v1',now()-interval '1 day',now(),true,'11111111-1111-1111-1111-111111111111',now(),'sneak') $$, 'P0001', 'a new run may not be created already approved: approval is', 'test 129: a run cannot be created already approved');
...
select denied($$ select finance.approve_dry_run((select id from dry),'ok') $$, 'P0001', 'approve_dry_run: run 08659f13-952f-4887-aeab-807efe01ed3a has no', 'test 104: approval without a completed report raises');
...
select denied($$ select finance.approve_dry_run((select id from dry),'again') $$, 'P0001', 'approve_dry_run: run 08659f13-952f-4887-aeab-807efe01ed3a is already', 'test 103: a second approval raises');
select denied($$ select finance.approve_dry_run((select id from dry),'  ') $$, 'P0001', 'approve_dry_run: a non-blank note is required', 'test 114: blank approval note raises');

-- test 105/115: approved evidence is frozen
select denied($$ update finance.reconciliation_runs set window_end=now()+interval '1 day' where id=(select id from dry) $$, 'P0001', 'approved evidence is frozen: run 08659f13-952f-4887-aeab-807efe01ed3a', 'test 105: window_end frozen after approval');
select denied($$ update finance.reconciliation_runs set report_version='r2' where id=(select id from dry) $$, 'P0001', 'approved evidence is frozen: run 08659f13-952f-4887-aeab-807efe01ed3a', 'test 105: report frozen after approval');
select denied($$ update finance.reconciliation_runs set implementation_version='v2' where id=(select id from dry) $$, 'P0001', 'approved evidence is frozen: run 08659f13-952f-4887-aeab-807efe01ed3a', 'test 105: implementation_version frozen after approval');
...
select denied($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id,resolution_status,resolved_at,resolved_by,resolution_note)
  values ('amount_mismatch',true,'ch_10','resolved',now(),'11111111-1111-1111-1111-111111111111','pre') $$, 'P0001', 'a new exception must be created open, got resolved', 'test 124: cannot create an exception already resolved');
select denied($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id,quarantined_at,quarantine_reason)
  values ('amount_mismatch',true,'ch_11',now(),'x') $$, 'P0001', 'a new exception may not be created with resolution, quarantine or', 'test 124: cannot create an exception already quarantined');
...
select denied($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id,dedup_key)
  values ('amount_mismatch',true,'ch_12','ATTACK') $$, '428C9', 'dedup_key', 'test 102: dedup_key cannot be supplied by the writer');
...
select denied($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id)
  values ('amount_mismatch',true,'ch_9') $$, '23505', 'reconciliation_exceptions_open_uq', 'test 79: a second open exception with the same identity is rejected');

-- test 99: provider_object_processing_failed shape
select denied($$ insert into finance.reconciliation_exceptions (kind,livemode,provider_object_id)
  values ('provider_object_processing_failed',true,'ch_20') $$, '23514', 'reconciliation_exceptions', 'test 99: processing-failure without detail rejected');
...
select denied($$ select finance.resolve_exception((select id from exc),'open','x') $$, 'P0001', 'resolve_exception: target must be resolved or dismissed, got open', 'test 119: resolving to open raises');
select denied($$ select finance.resolve_exception((select id from exc),'resolved','  ') $$, 'P0001', 'resolve_exception: a non-blank note is required', 'test 119: blank resolution note raises');
...
select denied($$ select finance.resolve_exception((select id from exc),'dismissed','again') $$, 'P0001', 'resolve_exception: exception 8f3c00ee-b736-4bbc-b61f-98c13e0ce60d is', 'test 119: repeat resolution raises');
