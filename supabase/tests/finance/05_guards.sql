...
select is((select count(*)::int from pg_proc p join pg_namespace n on n.oid=p.pronamespace
          ...
          'finding 5: no finance function is executable by PUBLIC [A5-013]');
...
select is((select count(*)::int from pg_proc p join pg_namespace n on n.oid=p.pronamespace
          ...
          'finding 5: no finance function ACL grants EXECUTE to PUBLIC [A5-014]');
...
select ok((select revoked_at is not null and revoked_by='11111111-1111-1111-1111-111111111111'::uuid
          ...
          'finding 4: revocation carries actor attribution computed internally [A5-005]');
select denied($$ select finance.revoke_payment_link((select id from lk)) $$, 'P0001', 'revoke_payment_link: link c57196c9-6d85-4680-91b0-b2af0d05c339 is', 'finding 4: a second revoke raises [A5-006]');
...
select is((select count(*)::int from information_schema.column_privileges
          ...
          'finding 4: no application role holds a direct UPDATE on revoked_at/revoked_by [A5-015]');
...
select lives_ok($$ select finance.quarantine_object((select id from e9)) $$,'nit 9: quarantine succeeds [A5-007]');
select lives_ok($$ select finance.resolve_exception((select id from e9),'resolved','done') $$,'nit 9: resolution wins over quarantine [A5-008]');
select denied($$ select finance.release_quarantine((select id from e9),'late') $$, 'P0001', 'release_quarantine: exception 629a56e7-3559-4e20-8180-9ffb63fd7c7c is', 'nit 9: release_quarantine rejects a non-open row, matching quarantine_object [A5-009]');
...
select is((select p.prosecdef::text from pg_proc p join pg_namespace n on n.oid=p.pronamespace
          ...
          'finding 10: f_balances is SECURITY INVOKER; a DEFINER here would leak every balance [A5-016]');
select is((select count(*)::int from pg_class c join pg_namespace n on n.oid=c.relnamespace
          ...
          'finding 10: every finance view sets security_invoker=true [A5-010]');
...
select is((select count(*)::int from pg_constraint c
          ...
          'finding 3: every finance -> auth.users FK is ON DELETE RESTRICT [A5-017]');
select denied($$ delete from auth.users where id='11111111-1111-1111-1111-111111111111' $$, '23503', 'users', 'finding 3: deleting an actor with financial history is REFUSED, not cascaded (D-073) [A5-011]');
select lives_ok($$ delete from auth.users where id='22222222-2222-2222-2222-222222222222' $$,
  'finding 3: an auth user with no financial attribution still deletes normally [A5-012]');
