...
select throws_ok($$ update finance.ledger_entries set amount_cents=1 $$,null,null,'req 5: ledger entries cannot be UPDATEd');
select throws_ok($$ delete from finance.ledger_entries $$,null,null,'req 5: ledger entries cannot be DELETEd');
...
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,provider_object_id,provider_payment_intent_id,occurred_at,livemode)
  select id,'stripe_payment',1,'stripe','ch_a','pi_zz',now(),true from ag $$,null,null,'req 44: duplicate provider object rejected');
...
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,provider_object_id,parent_entry_id,occurred_at,livemode)
  ...
  from (select id from finance.agreements where purpose='other') o,
       (select id from finance.ledger_entries where entry_type='stripe_payment' limit 1) p $$,
  null,null,'req 57: a parent in a different agreement is rejected');
select ok((select count(*)=1 from pg_constraint where conname='ledger_l5_not_self_parent'),'req 57: L5 self-parent CHECK exists');

-- req 30: refund shape
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,provider_object_id,occurred_at,livemode)
  select id,'refund',-100,'stripe','re_x',now(),true from ag $$,null,null,'req 30: refund without parent rejected');
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,provider_object_id,parent_entry_id,occurred_at,livemode)
  select a.id,'refund',100,'stripe','re_p',p.id,now(),true from ag a,(select id from finance.ledger_entries where entry_type='stripe_payment' limit 1) p $$,
  null,null,'req 30: refund with a positive amount rejected');
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,parent_entry_id,occurred_at,livemode)
  select a.id,'refund',-100,'stripe',p.id,now(),true from ag a,(select id from finance.ledger_entries where entry_type='stripe_payment' limit 1) p $$,
  null,null,'req 30: stripe refund without provider_object_id rejected');
...
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,provider_object_id,parent_entry_id,occurred_at,livemode)
  select a.id,'refund',-40000,'stripe','re_3',p.id,now(),true from ag a,(select id from finance.ledger_entries where entry_type='stripe_payment' limit 1) p $$,
  null,null,'req 49: a refund exceeding the settled amount is rejected');

-- req 51: refund may not target a refund
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,provider_object_id,parent_entry_id,occurred_at,livemode)
  select a.id,'refund',-100,'stripe','re_4',r.id,now(),true from ag a,(select id from finance.ledger_entries where entry_type='refund' limit 1) r $$,
  null,null,'req 51: a refund may not target a refund');
...
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,parent_entry_id,occurred_at,livemode,reason,recorded_by)
  select a.id,'reversal',999,'stripe',r.id,now(),true,'x','11111111-1111-1111-1111-111111111111' from ag a,(select id from finance.ledger_entries where entry_type='refund' limit 1) r $$,
  null,null,'req 52: a reversal that does not negate its parent is rejected');
...
select throws_ok($$ insert into finance.ledger_entries(agreement_id,entry_type,amount_cents,source,external_method,occurred_at,livemode,reason,recorded_by_system,legacy_donation_id)
  select id,'external_payment',500,'external','cash',now(),true,'import','legacy_import','dddddddd-0000-0000-0000-00000000000d' from ag $$,
  null,null,'req 59: re-running the import cannot duplicate');
...
select throws_ok($$ insert into finance.agreement_amounts(agreement_id,amount_cents,effective_at,reason,actor_id)
  select id,-5,now(),'neg','11111111-1111-1111-1111-111111111111' from ag $$,null,null,'req 27: a negative amount is rejected');
select throws_ok($$ insert into finance.agreement_amounts(agreement_id,amount_cents,effective_at,reason,actor_id)
  select id,5,now(),'   ','11111111-1111-1111-1111-111111111111' from ag $$,null,null,'req 27: a blank reason is rejected');
...
select throws_ok($$ insert into finance.agreement_lifecycle_events(agreement_id,from_status,to_status,reason,actor_id)
  select id,null,'draft','again','11111111-1111-1111-1111-111111111111' from ag $$,null,null,'req 19: only one initial event per agreement');
select throws_ok($$ insert into finance.agreement_lifecycle_events(agreement_id,from_status,to_status,reason,actor_id)
  select id,'draft','fulfilled','skip','11111111-1111-1111-1111-111111111111' from ag $$,null,null,'req 20: draft->fulfilled is rejected');
...
select throws_ok($$ insert into finance.checkout_sessions(agreement_id,idempotency_key,amount_cents,livemode,expires_at,status)
  select id,'k9',5000,true,now()+interval '1 hour','open' from ag $$,null,null,'req 34: non-creating status without stripe_session_id rejected');
select throws_ok($$ insert into finance.checkout_sessions(agreement_id,idempotency_key,amount_cents,livemode,expires_at)
  select id,'k10',0,true,now()+interval '1 hour' from ag $$,null,null,'req 33: amount_cents must be > 0');

-- req 73: payment_links status CHECKs
select throws_ok($$ insert into finance.payment_links(agreement_id,token_hash,expires_at,created_by,status)
  select id,'t1',now()+interval '1 day','11111111-1111-1111-1111-111111111111','creating' from ag $$,null,null,'req 73: creating without claimed_at rejected');
select throws_ok($$ insert into finance.payment_links(agreement_id,token_hash,expires_at,created_by,status,consumed_at)
  select id,'t2',now()+interval '1 day','11111111-1111-1111-1111-111111111111','consumed',now() from ag $$,null,null,'req 73: consumed without session rejected');

-- req 77/78/85/86: run constraints
select throws_ok($$ insert into finance.reconciliation_runs(livemode,implementation_version,window_start,window_end,dry_run)
  values (true,'v1',now(),now()-interval '1 hour',true) $$,null,null,'req 77: window_end <= window_start rejected');
...
select throws_ok($$ insert into finance.reconciliation_runs(livemode,implementation_version,window_start,window_end,dry_run)
  values (true,'v1',now()-interval '1 day',now(),true) $$,null,null,'req 78: a second running run for the same livemode is rejected');
...
select throws_ok($$ insert into finance.reconciliation_runs(livemode,implementation_version,window_start,window_end,dry_run,authorized_by_run_id)
  values (true,'v1',now()-interval '1 day',now(),true,(select id from finance.reconciliation_runs limit 1)) $$,
  null,null,'req 86: a dry run may not cite an authorization');
select throws_ok($$ insert into finance.reconciliation_runs(livemode,implementation_version,window_start,window_end,dry_run)
  values (true,'v1',now()-interval '1 day',now(),false) $$,null,null,'req 86: a writing run without authorization is rejected');

-- req 94/95: dry-run write and report constraints
select throws_ok($$ insert into finance.reconciliation_runs(livemode,implementation_version,window_start,window_end,dry_run,exceptions_created)
  values (false,'v2',now()-interval '1 day',now(),true,5) $$,null,null,'req 94: a dry run with non-zero real writes is rejected');
select throws_ok($$ update finance.reconciliation_runs set report_completed_at=now() where dry_run and livemode $$,
  null,null,'req 95: report_completed_at without the report columns is rejected');

-- req 82/87/109/117/127: exception constraints
select throws_ok($$ insert into finance.reconciliation_exceptions(kind,livemode,provider_object_id,first_detected_at,last_detected_at)
  values ('amount_mismatch',true,'ch_z',now(),now()-interval '1 hour') $$,null,null,'req 82: last_detected_at < first_detected_at rejected');
...
select throws_ok($$ select finance.quarantine_object((select id from q)) $$,null,null,'req 110: wrong kind cannot be quarantined');
...
select throws_ok($$ select finance.quarantine_object((select id from q2)) $$,null,null,'req 110: a first failure cannot quarantine');
...
select throws_ok($$ select finance.quarantine_object((select id from q2)) $$,null,null,'req 110: an already-quarantined row cannot be re-quarantined');
...
select throws_ok($$ select finance.quarantine_object((select id from q2)) $$,null,null,'req 112: re-quarantine requires three fresh failures');
