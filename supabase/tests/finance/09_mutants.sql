...
select denied($$ insert into finance.payment_links(agreement_id,token_hash,status,expires_at,created_by,revoked_at,revoked_by)
  select id,'tok_forge','revoked',now()+interval '1 day','11111111-1111-1111-1111-111111111111',now(),'11111111-1111-1111-1111-111111111111' from ag $$, 'P0001', 'a new payment link must be created active, got revoked', 'mutant kill: a link cannot be INSERTed already revoked with forged attribution [A9-009]');
select denied($$ insert into finance.payment_links(agreement_id,token_hash,status,expires_at,created_by,claimed_at)
  select id,'tok_claim','creating',now()+interval '1 day','11111111-1111-1111-1111-111111111111',now() from ag $$, 'P0001', 'a new payment link must be created active, got creating', 'mutant kill: a link cannot be INSERTed already claimed [A9-001]');
select denied($$ insert into finance.payment_links(agreement_id,token_hash,expires_at,created_by,attempt_count)
  select id,'tok_att',now()+interval '1 day','11111111-1111-1111-1111-111111111111',5 from ag $$, 'P0001', 'a new payment link must start with attempt_count = 0', 'mutant kill: a link cannot be INSERTed with a non-zero attempt_count [A9-002]');
...
select lives_ok($$ select finance.revoke_payment_link((select id from lk)) $$,'a founder revokes the link [A9-003]');
select denied($$ update finance.payment_links set status='active' where id=(select id from lk) $$, 'P0001', 'payment link af750372-6886-44c0-8804-d62b69f035c9 is revoked;', 'mutant kill: a revoked link cannot be reactivated [A9-004]');
select denied($$ update finance.payment_links set revoked_by='11111111-1111-1111-1111-111111111111', revoked_at=now()-interval '1 day' where id=(select id from lk) $$, 'P0001', 'revocation attribution on link af750372-6886-44c0-8804-d62b69f035c9', 'mutant kill: revocation attribution is frozen [A9-005]');
...
select denied($$ insert into finance.reconciliation_runs
  ...
  values (true,'v1',now()-interval '1 day',now(),true,'completed',true,now(),
          0,0,'{}'::jsonb,'r',now(),
          '11111111-1111-1111-1111-111111111111',now(),'forged') $$, 'P0001', 'a new run may not be created already approved: approval is', 'mutant kill: a run cannot be INSERTed already approved even with a complete report [A9-010]');
...
select is((select count(*)::int from pg_trigger t join pg_class c on c.oid=t.tgrelid
          ...
          'mutant kill: the deferred agreement-completeness constraint exists and is DEFERRABLE [A9-011]');
savepoint sp_defer;
...
select denied($$ set constraints finance.agreement_has_lifecycle immediate $$, 'P0001', 'agreement 2d9accad-e42f-4e0b-a667-90b06df89328 must have exactly one', 'an agreement with no initial lifecycle event fails the deferred check [A9-006]');
...
select is((select current_status::text from finance.v_agreement_lifecycle where agreement_id=(select id from ag)),
  'active','mutant kill: the view reports the LATEST event, not the earliest [A9-007]');
select is((select since from finance.v_agreement_lifecycle where agreement_id=(select id from ag)),
  ...
  'mutant kill: the view''s `since` is the latest occurred_at [A9-008]');
...
select lives_ok($$ insert into finance.agreement_lifecycle_events(agreement_id,from_status,to_status,reason,actor_id)
  ...
  'D-074: enforcement accepts a transition from the state the view reports [A9-012]');
...
select denied($$ insert into finance.agreement_lifecycle_events(agreement_id,from_status,to_status,reason,actor_id)
  select id,'draft','canceled','stale','11111111-1111-1111-1111-111111111111' from ag $$, 'P0001', 'stale transition: from_status draft but current status is active', 'D-074: enforcement rejects a transition from the superseded state [A9-013]');
