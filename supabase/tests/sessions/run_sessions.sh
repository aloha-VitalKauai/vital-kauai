#!/usr/bin/env bash
# Sessions V1 (Builds 1 + 2 + default program grant) — full verification
...
# of apply order: the grant automation, then Build 2 (holds reference
...
for m in "$MIG_V1" "$MIG_V2" "$MIG_V3"; do
...
echo "== reversibility: roll back (v3, v2, v1), then re-apply all =="
