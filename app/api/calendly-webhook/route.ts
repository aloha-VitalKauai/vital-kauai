...
// ---------------------------------------------------------------------------
// verified is true only when the signature was actually checked against a
// configured key and matched — the series system (sessions Build 2+) fails
// closed on anything less. valid keeps the endpoint's long-standing
// compatibility behavior: with no key configured, ordinary processing
// continues (and logs the gap) so legacy reconciliation never breaks.
function verifySignature(
  ...
  if (signingKeys.length === 0) {
    ...
    return { valid: true, verified: false } // TODO: set CALENDLY_WEBHOOK_SIGNING_KEY in production to enforce
  }
  ...
  if (!parts.t || !parts.v1) return { valid: false, verified: false }

  const provided = Buffer.from(String(parts.v1), 'utf8')
  const matched = signingKeys.some(key => {
    const expected = Buffer.from(
      createHmac('sha256', key).update(`${parts.t}.${rawBody}`).digest('hex'),
      'utf8',
    )
    return expected.length === provided.length && timingSafeEqual(expected, provided)
  })
  return { valid: matched, verified: matched }
}
...
export async function POST(req: NextRequest) {
  ...
  // Series creation inside requires signature.verified — an unverified
  // payload records bookings as before but can never spawn a series.
