...
import { type WeeklyCheckinRow } from "./MemberWeeklyCheckins";
...
export default async function MemberProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (profileByEmail) {
    const [{ data: pre }, { data: post }, { data: mj }, { data: checkins }] = await Promise.all([
      ...
      // Weekly check-ins — the member wrote these for the care team to read,
      // so unlike the journal they carry no sharing gate. Submitted only,
      // newest first.
      supabase
        .from("member_checkins")
        .select("id, week_number, submitted_at, questions_snapshot, responses")
    ...
    preProgress = pre;
    postProgress = post;
    weeklyCheckins = (checkins ?? []) as WeeklyCheckinRow[];
