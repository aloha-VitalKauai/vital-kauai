...
const PRIORITY_COLORS: Record<string, string> = {
  critical:C.crit, high:C.high, medium:C.amber, low:C.muted,
}
...
const TASK_TYPE_LABELS: Record<string, string> = {
  medical_clearance:'Medical clearance', cardiac_clearance:'Cardiac clearance',
  labs_missing:'Labs missing', followup_overdue:'Follow-up overdue',
  contraindication_review:'Contraindication review', ceremony_readiness:'Ceremony readiness',
  agreement_missing:'Agreement missing', deposit_missing:'Deposit missing',
  guide_outreach:'Guide outreach',
}
const ALERT_TO_TASK_TYPE: Record<string, string> = {
...
function pill(color: string, bg: string) {
  return {
    display:'inline-block' as const, fontSize:10, fontWeight:600,
...
function StagePill({ stage }: { stage:string }) {
  ...
  return <span style={pill(m.color, m.color+'18')}>{m.label}</span>
}
...
function Avatar({ name, size=36 }: { name:string; size?:number }) {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:'50%', background:c+'2A', border:`1px solid ${c}50`, color:c, display:'flex', alignItems:'center', justifyContent:'center', fontSize:size*.32, fontWeight:600, flexShrink:0 }}>
...
function Btn({ label, color, onClick, small }: { label:string; color:string; onClick:()=>void; small?:boolean }) {
  return (
    <button onClick={e => { e.stopPropagation(); onClick() }} style={{ fontSize:small?9:10, color, background:'transparent', border:`0.5px solid ${color}55`, borderRadius:6, padding:small?'2px 8px':'3px 10px', cursor:'pointer', letterSpacing:'.05em', fontWeight:500, whiteSpace:'nowrap' }}>
      {label}
    </button>
  )
}
...
const ChartTip = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background:C.card, border:`0.5px solid ${C.border}`, borderRadius:8, padding:'8px 14px' }}>
      ...
      {payload.map((p: any) => <div key={p.dataKey} style={{ fontSize:12, color:p.color, fontWeight:500 }}>{p.name}: {p.value?.toFixed(1)}</div>)}
    </div>
  )
}
...
function AssignEditor({ current, onSave, onCancel }: { current:string|null; onSave:(v:string)=>void; onCancel:()=>void }) {
  ...
  const inp = { fontSize:11, background:C.faint, border:`0.5px solid ${C.hi}`, borderRadius:6, padding:'3px 8px', color:C.text, width:120, outline:'none' }
  return (
...
/* ── CREATE TASK FORM ────────────────────────────────── */
function CreateTaskForm({ alert, memberName, onCreate, onCancel }: {
  alert: any; memberName: string; onCreate: (d:any)=>void; onCancel:()=>void
}) {
  const defaultType = ALERT_TO_TASK_TYPE[alert.alert_type] ?? 'guide_outreach'
  const [taskType, setTaskType] = useState(defaultType)
  const [assignee, setAssignee] = useState('')
  const [dueDate,  setDueDate]  = useState('')
  const inp = { fontSize:11, background:C.faint, border:`0.5px solid ${C.hi}`, borderRadius:6, padding:'4px 8px', color:C.text, width:'100%', outline:'none' }
  const lbl = { fontSize:9, color:C.dim, letterSpacing:'.09em', textTransform:'uppercase' as const, marginBottom:4, display:'block' as const }
  return (
    <div onClick={e=>e.stopPropagation()} style={{ marginTop:12, paddingTop:12, borderTop:`0.5px solid ${C.border}`, display:'flex', flexDirection:'column', gap:10 }}>
      <div style={{ fontSize:11, color:C.amber, fontWeight:500, marginBottom:2 }}>Create task for {memberName}</div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
        <div>
          <span style={lbl}>Task type</span>
          <select value={taskType} onChange={e=>setTaskType(e.target.value)} style={{ ...inp, appearance:'none' as any }}>
            {Object.entries(TASK_TYPE_LABELS).map(([v,l]) => <option key={v} value={v}>{l}</option>)}
          </select>
        </div>
        <div>
          <span style={lbl}>Assign to</span>
          <input value={assignee} onChange={e=>setAssignee(e.target.value)} placeholder="Guide name" style={inp} />
        </div>
        <div>
...
/* ── TASK CARD ─────────────────────────────────────── */
function TaskCard({ task, onStatusChange, onAssign }: {
  task: any; onStatusChange:(id:string,s:string)=>void; onAssign:(id:string,name:string)=>void
}) {
    ...
    <div style={{ background:C.faint, borderRadius:8, padding:'10px 12px', border:`0.5px solid ${task.status==='in_progress'?C.blue+'44':C.border}`, borderLeft:`2px solid ${PRIORITY_COLORS[task.priority]}` }}>
      <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:12, fontWeight:500, color:C.text, marginBottom:3 }}>{task.title}</div>
          <div style={{ display:'flex', gap:10, flexWrap:'wrap', fontSize:10, color:C.dim, alignItems:'center' }}>
          ...
          {assigning && (
            <div style={{ marginTop:8 }}>
              <AssignEditor
                current={task.owner_name}
                onSave={name => { onAssign(task.id, name); setAssigning(false) }}
...
/* ── ALERT CARD ─────────────────────────────────────── */
function AlertCard({ alert, memberName, onAcknowledge, onCreateTask, taskAlreadyExists }: {
  alert:any; memberName:string; onAcknowledge:(id:string)=>void
  onCreateTask:(a:any,d:any)=>void; taskAlreadyExists:boolean
}) {
    ...
    <div style={{ background:C.card, border:`0.5px solid ${sm.color}33`, borderLeft:`3px solid ${sm.color}`, borderRadius:12, padding:'13px 16px', opacity:alert.acknowledged?.6:1 }}>
      <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:5, flexShrink:0, flexWrap:'wrap', justifyContent:'flex-end' }}>
          {!alert.acknowledged && (
            <>
              {!taskAlreadyExists && !showForm && <Btn label="Create Task"  color={C.amber} onClick={() => setShowForm(true)}           small />}
              {taskAlreadyExists                && <span style={{ fontSize:10, color:C.low, alignSelf:'center' }}>✓ Task exists</span>}
      ...
      {showForm && (
        <CreateTaskForm
          ...
          onCreate={d => { onCreateTask(alert, d); setShowForm(false) }}
...
/* ── TODAY PANEL ──────────────────────────────────────── */
function TodayPanel({ tasks, activity, onStatusChange, onAssign, memberLookup }: {
  tasks:any[]; activity:any[]; onStatusChange:(id:string,s:string)=>void
  onAssign:(id:string,n:string)=>void; memberLookup:Record<string,any>
}) {
  const [collapsed, setCollapsed] = useState(false)
  const open = tasks.filter(t => t.status === 'open' || t.status === 'in_progress')
  if (!open.length && !activity.length) return null
  return (
    <div style={{ background:C.faint, border:`0.5px solid ${C.hi}`, borderLeft:`3px solid ${C.terra}`, borderRadius:12, padding:'14px 18px', marginBottom:18 }}>
      ...
      {!collapsed && activity.length > 0 && (
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:12, padding:'8px 10px', background:C.bg, borderRadius:8, border:`0.5px solid ${C.border}` }}>
      ...
      {!collapsed && (
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {open.sort((a,b) => (['critical','high','medium','low'].indexOf(a.priority)) - (['critical','high','medium','low'].indexOf(b.priority))).map((t,i) => (
...
export default function OpsDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      for (const r of riskData ?? []) rMap[r.member_id] = r
      setRiskScores(rMap)

      setAlerts(alertsData ?? [])
      setOverdueFu(overdueData ?? [])

      const oMap: Record<string,any> = {}
      for (const o of outcomesData ?? []) oMap[o.member_id] = o
      setOutcomes(oMap)
  ...
  const updateTaskStatus = useCallback(async (taskId: string, newStatus: string) => {
    ...
    if (newStatus === 'done') {
      await supabase.from('member_timelines').insert({
        member_id:   task.member_id,
        event_type:  'task_completed',
        event_title: `Task completed: ${task.title}`,
        event_detail: `Type: ${TASK_TYPE_LABELS[task.task_type] ?? task.task_type}`,
        event_date:  new Date().toISOString(),
        is_system:   false,
        actor_name:  task.owner_name ?? 'Guide',
        source_table:'ops_tasks',
        source_id:   taskId,
      })
    }
    ...
    setActivity(prev => [{
      text: `${firstName} · "${task.title}" ${newStatus === 'done' ? 'completed' : newStatus === 'in_progress' ? 'started' : 'reopened'}`,
      color: newStatus === 'done' ? C.low : newStatus === 'in_progress' ? C.blue : C.muted,
      ts: Date.now(),
    }, ...prev.slice(0, 4)])
  }, [tasks, members])
  ...
  const assignTask = useCallback(async (taskId: string, ownerName: string) => {
    ...
    if (ownerName) {
      setActivity(prev => [{
        text: `${member?.full_name?.split(' ')[0]} · task assigned to ${ownerName}`,
        color: C.amber, ts: Date.now(),
      }, ...prev.slice(0, 4)])
    }
  }, [tasks, members])
  ...
  const createTaskFromAlert = useCallback(async (alert: any, taskData: any) => {
    const supabase = createClient()
    const member = members.find(m => m.member_id === alert.member_id)
    const newTask = {
      member_id:    alert.member_id,
      task_type:    taskData.task_type,
      title:        TASK_TYPE_LABELS[taskData.task_type] ?? taskData.task_type,
      priority:     alert.severity === 'critical' ? 'critical' : alert.severity === 'high' ? 'high' : 'medium',
      status:       'open',
      owner_name:   taskData.owner_name,
      due_date:     taskData.due_date,
      source_view:  'ops_alerts',
      source_reason: alert.alert_type,
    }
    const { data: created } = await supabase.from('ops_tasks').insert(newTask).select().single()
    if (created) {
      setTasks(prev => [...prev, created])
      // Link alert to task
      await supabase.from('ops_alerts').update({ source_id: created.id }).eq('id', alert.id)
      setAlerts(prev => prev.map(a => a.id === alert.id ? { ...a, linked_task_id: created.id } : a))
    }
    setActivity(prev => [{
      text: `Task created · ${member?.full_name?.split(' ')[0]} · ${TASK_TYPE_LABELS[taskData.task_type]}`,
      color: C.amber, ts: Date.now(),
    }, ...prev.slice(0, 4)])
  }, [members])
  ...
  const ackAlert = useCallback(async (id: string) => {
    ...
    await supabase.from('ops_alerts').update({
      acknowledged: true,
    ...
    const member = members.find(m => m.member_id === alert?.member_id)
    setActivity(prev => [{
      text: `Alert acknowledged · ${member?.full_name?.split(' ')[0] ?? 'member'}`,
      color: C.muted, ts: Date.now(),
    }, ...prev.slice(0, 4)])
  }, [alerts, members])
  ...
  const memberLookup = useMemo(() => {
    ...
    for (const m of members) map[m.member_id] = m
    return map
  }, [members])

  const memberTasksMap = useMemo(() => {
    const map: Record<string,any[]> = {}
    for (const t of tasks.filter(t => t.status !== 'done' && t.status !== 'canceled')) {
      if (!map[t.member_id]) map[t.member_id] = []
      map[t.member_id].push(t)
    }
    return map
  }, [tasks])
  ...
  const sortedMembers = [...members].sort((a,b) =>
    (STAGE_META[a.pipeline_stage]?.order ?? 9) - (STAGE_META[b.pipeline_stage]?.order ?? 9)
  )
  const visibleRisk = riskFilter === 'all' ? sortedMembers : sortedMembers.filter(m => {
    const level = riskScores[m.member_id]?.risk_level ?? m.risk_level
    return level === riskFilter
  })

  const cohortTrend = [
    { label:'Baseline', phq9:21.0, gad7:15.0 },
    { label:'1 wk',     phq9:12.5, gad7:9.8  },
    { label:'1 mo',     phq9:9.0,  gad7:7.0  },
  ]

  if (loading) {
    return (
      <div style={{ margin:'-2rem', minHeight:'calc(100vh - 101px)', background:C.bg, display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div style={{ fontSize:12, color:C.muted, letterSpacing:'.1em', textTransform:'uppercase' }}>Loading ops data…</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ margin:'-2rem', minHeight:'calc(100vh - 101px)', background:C.bg, fontFamily:'var(--font-jost, sans-serif)', color:C.text }}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}} ::-webkit-scrollbar{width:3px} ::-webkit-scrollbar-track{background:${C.bg}} ::-webkit-scrollbar-thumb{background:${C.border};border-radius:2px} button:focus{outline:none} select{background:${C.faint};color:${C.text}}`}</style>
      ...
      <div style={{ padding:'20px 24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:8, marginBottom:16 }}>
          {([
            ...
            ['Open Tasks',          tasks.filter(t=>t.status!=='done'&&t.status!=='canceled').length, `${tasks.filter(t=>t.priority==='critical'||t.priority==='high').length} high priority`, (critCount||highCount)?C.high:null],
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:14 }}>
          <div>
            ...
            {tab === 'pipeline' && (
              <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
                {sortedMembers.map(m => {
                  ...
                  return (
                    <div key={m.member_id} style={{ background:open?C.surf:C.card, border:`0.5px solid ${open?C.hi:C.border}`, borderRadius:12, overflow:'hidden', transition:'all .15s' }}>
                      <div onClick={() => setExpanded(v => v===m.member_id?null:m.member_id)} style={{ display:'flex', alignItems:'center', gap:11, padding:'13px 16px', cursor:'pointer' }}>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          <div style={{ display:'flex', gap:12, flexWrap:'wrap', fontSize:11, color:C.muted, alignItems:'center' }}>
                            ...
                            {/* INLINE OUTCOMES */}
                            {outcome?.phq9_delta!=null && (
                              <span style={{ display:'flex', alignItems:'center', gap:3, color:outcome.phq9_delta<0?C.low:C.high }}>
                                <span style={{ fontSize:10 }}>PHQ-9</span>
                                <span style={{ fontWeight:600 }}>{outcome.phq9_delta>0?'+':''}{outcome.phq9_delta}</span>
                              </span>
                            )}
                            {outcome?.gad7_delta!=null && (
                              <span style={{ display:'flex', alignItems:'center', gap:3, color:outcome.gad7_delta<0?C.low:C.high }}>
                                <span style={{ fontSize:10 }}>GAD-7</span>
                                <span style={{ fontWeight:600 }}>{outcome.gad7_delta>0?'+':''}{outcome.gad7_delta}</span>
                              </span>
                            )}
                            {outcome?.regulation_delta!=null && (
                              <span style={{ display:'flex', alignItems:'center', gap:3, color:C.low }}>
                                <span style={{ fontSize:10 }}>Reg</span>
                                <span style={{ fontWeight:600 }}>+{outcome.regulation_delta}</span>
                              </span>
                            )}
                        ...
                        <div style={{ textAlign:'right', flexShrink:0 }}>
                          {m.program_price && <div style={{ fontSize:11, fontWeight:600, color:C.dim }}>${(m.program_price/1000).toFixed(0)}k</div>}
                          <div style={{ fontSize:10, color:C.dim, marginTop:1 }}>score {rs.risk_score ?? 0}</div>
                        </div>
                        ...
                        <div style={{ borderTop:`0.5px solid ${C.border}`, padding:'14px 16px' }}>
                          {rFactors.length > 0 && (
                            <div style={{ marginBottom:12, display:'flex', gap:6, flexWrap:'wrap' }}>
                              {rFactors.map((r:string) => <span key={r} style={pill(rLevel==='high'||rLevel==='critical'?C.high:C.amber, rLevel==='high'||rLevel==='critical'?C.highBg:C.medBg)}>{REASON_LABELS[r]??r}</span>)}
                            </div>
                          )}
                          ...
                          {sub==='tasks' && (
                            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                              {mTasks.length===0
                                ? <div style={{ fontSize:12, color:C.dim, padding:'8px 0' }}>No open tasks.</div>
                                : mTasks.map(t => <TaskCard key={t.id} task={t} onStatusChange={updateTaskStatus} onAssign={assignTask} />)
                              }
                            </div>
                          )}
                          {sub==='timeline' && <div style={{ fontSize:12, color:C.dim, padding:'8px 0' }}>Timeline events appear as members progress.</div>}
                          {!sub && m.primary_intention && (
                            <div style={{ padding:'9px 12px', background:C.faint, borderRadius:8, border:`0.5px solid ${C.border}`, borderLeft:`2px solid ${C.terra}` }}>
                              <div style={{ fontSize:9, color:C.dim, letterSpacing:'.09em', textTransform:'uppercase', marginBottom:3 }}>Intention</div>
                              <div style={{ fontSize:12, color:C.muted, fontStyle:'italic' }}>"{m.primary_intention}"</div>
                            </div>
                          )}
                        </div>
                      )}
            ...
            {tab === 'risk' && (
              <div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {[...visibleRisk].sort((a,b) => (riskScores[b.member_id]?.risk_score??0) - (riskScores[a.member_id]?.risk_score??0)).map(m => {
                    const rs     = riskScores[m.member_id] ?? {}
                    const rLevel = rs.risk_level ?? m.risk_level ?? 'low'
                    const rc     = RISK_META[rLevel] ?? RISK_META.low
                    const rFactors: string[] = rs.risk_factors ?? m.risk_factors ?? []
                    const mTasks = memberTasksMap[m.member_id] ?? []
                    return (
                      <div key={m.member_id} style={{ background:C.card, border:`0.5px solid ${C.border}`, borderLeft:`3px solid ${rc.color}`, borderRadius:12, padding:'14px 16px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:rFactors.length||mTasks.length?12:0 }}>
                          <Avatar name={m.full_name} size={30} />
                          <div style={{ flex:1 }}>
                            <div style={{ fontSize:13, fontWeight:500, marginBottom:3 }}>{m.full_name}</div>
                            <div style={{ display:'flex', alignItems:'center', gap:8 }}><StagePill stage={m.pipeline_stage}/>{m.ceremony_date&&<span style={{ fontSize:11, color:C.muted }}>{m.ceremony_date}</span>}</div>
                          </div>
                          <div style={{ textAlign:'right' }}>
                            <div style={{ fontSize:22, fontWeight:600, color:rc.color, fontFamily:'var(--font-cormorant-garamond, serif)', lineHeight:1 }}>{rs.risk_score??0}</div>
                            <div style={{ fontSize:9, color:C.dim, letterSpacing:'.08em', textTransform:'uppercase', marginTop:2 }}>score</div>
                          </div>
                          <RiskPill level={rLevel} />
                        </div>
                        ...
                        {mTasks.length > 0 && <div style={{ display:'flex', flexDirection:'column', gap:5, marginTop:rFactors.length?10:0 }}>{mTasks.map(t=><TaskCard key={t.id} task={t} onStatusChange={updateTaskStatus} onAssign={assignTask}/>)}</div>}
                        {!rFactors.length && !mTasks.length && <div style={{ fontSize:11, color:C.low }}>✓ No active risk flags or open tasks</div>}
                      </div>
                    )
                  })}
                </div>
            ...
            {tab === 'outcomes' && (
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                <div style={{ background:C.card, border:`0.5px solid ${C.border}`, borderRadius:12, padding:'16px 18px' }}>
                  ...
                  <ResponsiveContainer width="100%" height={160}>
                    <AreaChart data={cohortTrend} margin={{ top:4, right:4, left:-22, bottom:0 }}>
                      <defs>
                        <linearGradient id="gP" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={C.blue} stopOpacity={.2}/><stop offset="95%" stopColor={C.blue} stopOpacity={0}/></linearGradient>
                        <linearGradient id="gG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={C.purple} stopOpacity={.2}/><stop offset="95%" stopColor={C.purple} stopOpacity={0}/></linearGradient>
                      </defs>
                ...
                {members.map((m:any) => {
                  const o = outcomes[m.member_id]
                  const hasData = o?.phq9_delta != null
                  const rcColor = o?.phq9_delta < 0 ? C.low : o?.phq9_delta > 0 ? C.high : C.dim
                  return (
                    <div key={m.member_id} style={{ background:C.card, border:`0.5px solid ${C.border}`, borderRadius:12, padding:'13px 16px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:hasData?10:0 }}>
                        <Avatar name={m.full_name} size={30} />
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:13, fontWeight:500, marginBottom:2 }}>{m.full_name}</div>
                          {o?.latest_timepoint ? <span style={pill(C.blue,C.blueBg)}>{o.latest_timepoint.replace(/_/g,' ')}</span> : <span style={{ fontSize:11, color:C.dim }}>No follow-up data yet</span>}
                        </div>
                        {hasData && (
                          <div style={{ textAlign:'right' }}>
                            <div style={{ fontSize:18, fontWeight:600, color:rcColor, fontFamily:'var(--font-cormorant-garamond, serif)', lineHeight:1 }}>{o?.phq9_pct_improvement?.toFixed(0)}%</div>
                            <div style={{ fontSize:9, color:C.dim, textTransform:'uppercase', letterSpacing:'.07em', marginTop:1 }}>{o?.phq9_response_class?.replace(/_/g,' ')}</div>
                          </div>
                        )}
                      </div>
                      {hasData && (
                        <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:6 }}>
                          {([
                            ['PHQ-9',      `${o.phq9_delta>0?'+':''}${o.phq9_delta}`,             o.phq9_delta<0?C.low:C.high],
                            ['GAD-7',      `${o.gad7_delta>0?'+':''}${o.gad7_delta}`,             o.gad7_delta<0?C.low:C.high],
                            ['Regulation', o.regulation_delta!=null?`+${o.regulation_delta}`:'—', C.low],
                            ['Practice',   o.practice_days!=null?`${o.practice_days}d/wk`:'—',    o.practice_days>=5?C.low:o.practice_days>=3?C.amber:C.muted],
                            ['Patterns',   o.pattern_returned===false?'Clear':o.pattern_returned===true?'Returned':'—', o.pattern_returned===false?C.low:C.amber],
                          ] as [string,string,string][]).map(([l,v,c]) => (
                            <div key={l} style={{ background:C.faint, borderRadius:7, padding:'7px 9px', border:`0.5px solid ${C.border}` }}>
                              <div style={{ fontSize:9, color:C.dim, letterSpacing:'.08em', textTransform:'uppercase', marginBottom:2 }}>{l}</div>
            ...
            {tab === 'alerts' && (
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                ...
                {[...alerts].filter(a=>a.is_active).sort((a,b) => ({critical:0,high:1,warning:2,info:3}[a.severity as string]??4) - ({critical:0,high:1,warning:2,info:3}[b.severity as string]??4)).map(a => {
                  const member = memberLookup[a.member_id]
                  const taskLinked = a.linked_task_id && tasks.find(t => t.id === a.linked_task_id)
                  return (
                    <AlertCard
                      key={a.id}
                      alert={a}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              ...
              {Object.entries(STAGE_META).sort((a,b)=>a[1].order-b[1].order).map(([stage,meta]) => {
                ...
                return (
                  <div key={stage} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:7 }}>
                    <div style={{ width:6, height:6, borderRadius:'50%', background:count>0?meta.color:C.dim, flexShrink:0 }} />
                    <div style={{ flex:1, fontSize:11, color:count>0?C.muted:C.dim }}>{meta.label}</div>
                    <span style={{ fontSize:11, fontWeight:600, color:count>0?C.text:C.dim, background:count>0?C.surf:'transparent', borderRadius:20, padding:'1px 7px', minWidth:18, textAlign:'center' }}>{count}</span>
                  </div>
                )
              })}
            </div>
            ...
            <div style={{ background:C.card, border:`0.5px solid ${C.border}`, borderRadius:12, padding:'14px 16px' }}>
              ...
              {upcoming.length===0
                ?<div style={{ fontSize:11, color:C.dim }}>None scheduled</div>
                :upcoming.map((c:any)=>(
                  <div key={c.member_id} style={{ display:'flex', alignItems:'center', gap:9, marginBottom:10 }}>
                    <Avatar name={c.full_name} size={26} />
                    <div style={{ flex:1 }}><div style={{ fontSize:12, fontWeight:500 }}>{c.full_name}</div><div style={{ fontSize:10, color:C.muted }}>{c.ceremony_date}</div></div>
                    <div style={{ fontSize:14, fontWeight:600, color:C.blue, fontFamily:'var(--font-cormorant-garamond, serif)' }}>{c.days_until_ceremony}d</div>
                  </div>
                ))
              }
            </div>
            ...
            <div style={{ background:C.card, border:`0.5px solid ${overdueFu.length?C.amber+'44':C.border}`, borderRadius:12, padding:'14px 16px' }}>
              <div style={{ fontSize:9, color:C.dim, letterSpacing:'.1em', textTransform:'uppercase', marginBottom:11 }}>
                Follow-ups{overdueFu.length>0&&<span style={{ color:C.amber, marginLeft:5 }}>· {overdueFu.length} overdue</span>}
              </div>
              {overdueFu.length===0
                ?<div style={{ fontSize:11, color:C.low }}>All current ✓</div>
                :overdueFu.map((f:any)=>(
                  <div key={`${f.member_id}_${f.timepoint}`} style={{ background:C.amberBg, border:`0.5px solid ${C.amber}33`, borderRadius:8, padding:'10px 11px', marginBottom:6 }}>
                    ...
                    <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
                      <button style={{ fontSize:10, color:C.amber, background:'transparent', border:`0.5px solid ${C.amber}55`, borderRadius:6, padding:'3px 9px', cursor:'pointer' }}>Send reminder</button>
            ...
            {totalRev > 0 && (
              <div style={{ background:C.card, border:`0.5px solid ${C.border}`, borderRadius:12, padding:'14px 16px' }}>
                ...
                {totalProfit > 0 && (
                  <div style={{ display:'flex', justifyContent:'space-between', paddingTop:9, borderTop:`0.5px solid ${C.border}` }}>
                    <div><div style={{ fontSize:9, color:C.dim, letterSpacing:'.08em', textTransform:'uppercase', marginBottom:2 }}>Net profit</div><div style={{ fontSize:15, fontWeight:600, color:C.low }}>${totalProfit.toLocaleString()}</div></div>
                    <div style={{ textAlign:'right' }}><div style={{ fontSize:9, color:C.dim, letterSpacing:'.08em', textTransform:'uppercase', marginBottom:2 }}>Margin</div><div style={{ fontSize:15, fontWeight:600, color:C.low }}>{Math.round((totalProfit/totalRev)*100)}%</div></div>
                  </div>
                )}
              </div>
            )}
