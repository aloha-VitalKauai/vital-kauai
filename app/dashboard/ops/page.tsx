...
export default function OpsDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [
        ...
        // PR 9 (D-086): canonical V2 only; the view carries its own founder boundary.
      ...
      setFinOverview((fo as any)?.[0] ?? null)
      setLoading(false)
    }
    load()
  }, [router])
  ...
  // PR 9 (D-086): every figure below is read straight from the canonical V2
  // overview. Operating margin is computed once in SQL and is NOT re-derived
  // here. A failed read leaves finOverview null, which renders as unavailable
  // rather than as zero.
  const finReady     = finOverview != null
  const contribRev   = (finOverview?.contribution_cents??0)/100
  const agreementsN  = finOverview?.active_agreements ?? 0
  const receivedRev  = (finOverview?.net_received_cents??0)/100
  const remainingRev = (finOverview?.remaining_cents??0)/100
  const marginRev    = (finOverview?.operating_margin_cents??0)/100
