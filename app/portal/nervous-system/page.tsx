    ...
    <div style={{ minHeight: "100vh", background: warmWhite, fontFamily: "'Jost', sans-serif", fontWeight: 300, color: ink }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 60px 120px" }}>
        ...
        <div style={{ background: "rgba(122,158,126,0.06)", border: "1px solid rgba(122,158,126,0.15)", borderRadius: 12, padding: "36px 40px", marginBottom: 48 }}>
          ...
          <p style={{ fontSize: 13, color: stone, lineHeight: 1.8, marginBottom: 24 }}>
            Peter Levine, the founder of Somatic Experiencing, teaches that everything we live through unfolds across five interwoven channels. Any moment of tracking may move through several of them.
          </p>
          {[
            { letter: "S", word: "Sensation", text: "The physical: warmth, coolness, tingling, pressure, weight, density, pulsing, streaming. The raw felt sense of the body itself." },
            ...
            <div key={ch.letter} style={{ display: "flex", gap: 20, padding: "16px 0", borderTop: `1px solid ${border}` }}>
              ...
              <div>
                <p style={{ fontSize: 14, fontWeight: 500, color: ink, marginBottom: 4 }}>{ch.word}</p>
                <p style={{ fontSize: 13, color: inkMid, lineHeight: 1.75 }} dangerouslySetInnerHTML={{ __html: ch.text }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0, marginBottom: 48 }}>
          {[
            ...
            <div key={step.num} style={{ padding: "28px 0", borderBottom: `1px solid ${border}` }}>
              ...
              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontWeight: 300, lineHeight: 1.35, marginBottom: 12 }}>{step.title}</p>
              <p style={{ fontSize: 13.5, color: inkMid, lineHeight: 1.85 }} dangerouslySetInnerHTML={{ __html: step.text }} />
            </div>
          ))}
        </div>

        {/* Vocabulary grid */}
        <span style={{ fontSize: 8.5, letterSpacing: "0.44em", textTransform: "uppercase", color: gold, display: "block", marginBottom: 10 }}>A Vocabulary for the Body</span>
        <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(22px, 2.6vw, 30px)", fontWeight: 300, lineHeight: 1.15, marginBottom: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 40 }}>
          {[
            { category: "Temperature", words: "warm · cool · hot · cold · icy · burning" },
            { category: "Quality", words: "tingling · buzzing · pulsing · still · heavy · light · tight · open · dense · airy" },
            { category: "Size & Shape", words: "small · vast · pointed · diffuse · wide · narrow · edged · shapeless" },
            { category: "Movement", words: "rising · falling · streaming · pooling · flowing · pulsing · stuck · trembling" },
            { category: "Texture", words: "sharp · smooth · rough · soft · prickly · velvety · grainy" },
            { category: "Direction", words: "forward · back · up · down · inward · outward · spiraling" },
          ].map((row) => (
            <div key={row.category} style={{ background: "rgba(122,158,126,0.04)", border: "1px solid rgba(122,158,126,0.12)", borderRadius: 8, padding: "20px 24px" }}>
              <p style={{ fontSize: 10, letterSpacing: "0.22em", textTransform: "uppercase", color: sage, fontWeight: 500, marginBottom: 8 }}>{row.category}</p>
              <p style={{ fontSize: 14, color: inkMid, lineHeight: 1.7, fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}>{row.words}</p>
            </div>
          ))}
        </div>
