  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FDFBF7", fontFamily: "'Jost', sans-serif", fontWeight: 300 }}>
      ...
      <div className="cj-actionbar" style={{ position: "sticky", top: "calc(60px + env(safe-area-inset-top))", zIndex: 40, background: "white", borderBottom: "1px solid rgba(28,43,30,0.1)", padding: "0 clamp(20px, 6vw, 60px)", display: "flex", alignItems: "center", height: 52, gap: 16 }}>
        ...
        <button onClick={() => window.print()} style={{ marginLeft: "auto", fontFamily: "'Jost', sans-serif", fontSize: 11, letterSpacing: "0.16em", textTransform: "uppercase", padding: "10px 18px", border: "1px solid rgba(28,43,30,0.18)", cursor: "pointer", background: "transparent", color: "#3D4D3F", fontWeight: 500 }}>
          Print Journal
        </button>
      </div>
        ...
        {PHASES.map((phase, pi) => {
          ...
          return (
            <div key={phase.key} id={phase.key} style={{ scrollMarginTop: 80 }}>
              ...
              {/* Phase header */}
              <div style={{ paddingTop: 64, paddingBottom: 24, borderBottom: "1px solid rgba(28,43,30,0.12)" }}>
                <span style={{ fontSize: 12, letterSpacing: "0.32em", textTransform: "uppercase", color: "#A87A2C", display: "block", marginBottom: 12, fontWeight: 600 }}>{phase.eyebrow}</span>
              ...
              {phase.weeks.map((week, wi) => {
                ...
                <div key={wi} style={{ marginTop: 56 }}>
                  ...
                  <div style={{ background: "rgba(122,158,126,0.06)", borderLeft: "3px solid #7A9E7E", padding: "20px 24px", marginBottom: 28 }}>
                    <span style={{ fontSize: 12, letterSpacing: "0.26em", textTransform: "uppercase", color: "#5C7A5F", display: "block", marginBottom: 6, fontWeight: 600 }}>
                      Week {wi + 1} · {week.code} · {week.theme}
                    </span>
                    {week.title && (
                      <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontWeight: 300, color: "#1A1A18", fontStyle: "italic", lineHeight: 1.3 }}>{week.title}</p>
                    )}
                  </div>

                  {/* Journal Prompts */}
                  <span style={{ fontSize: 11, letterSpacing: "0.3em", textTransform: "uppercase", color: "#5A5046", display: "block", marginBottom: 4, fontWeight: 600 }}>
                    Journal Prompts
                  </span>
                  ...
                  {showPne && (
                    <div style={{ marginTop: 28, padding: "26px 28px", background: "rgba(122,158,126,0.05)", border: "1px solid rgba(122,158,126,0.18)", borderLeft: "3px solid #7A9E7E" }}>
                      <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.28em", textTransform: "uppercase", color: "#5C7A5F", display: "block", marginBottom: 12 }}>
                        PNE Reflection
                      </span>
                      {pneDetail?.reflection ? (
                        ...
                        <p style={{ fontSize: 14, color: "#3D4D3F", lineHeight: 1.7, marginBottom: 12, fontStyle: "italic" }}>
                      ...
                      {(pneDetail?.reflectionThird || pneThirdEntry) && (
                        <>
                          {pneDetail?.reflectionThird && (
                            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontWeight: 300, color: "#1A1A18", lineHeight: 1.35, marginTop: 24, marginBottom: 12 }}>
                              {pneDetail.reflectionThird}
                            </p>
                          )}
                          <textarea
                            className="cj-textarea"
                            value={pneThirdEntry}
                            onChange={(e) => update(pneThirdKey, e.target.value)}
                            placeholder="Write freely..."
                            style={{
                              width: "100%",
                              minHeight: 130,
